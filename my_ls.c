...
void print_details(const char *path, struct stat *file_stat);
void print_inode(struct stat *file_stat);
void recursive_ls(const char *path, int show_all, int show_details, int show_inode, int show_size, int depth);
...
void ls(const char *path, int show_all, int show_details, int recursive, int sort_time, int reverse, int show_inode, int show_size) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (show_details) {
            print_details(full_path, &file_stat);  // 显示文件详细信息
        }
        if (show_inode) {
            print_inode(&file_stat);  // 显示文件的 inode
        }
        if (show_size) {
...
void print_details(const char *path, struct stat *file_stat) {
    ...
    // 显示文件大小和其他信息
    printf(" Size: %ld bytes\n", file_stat->st_size);
    printf("Last modified: %s", ctime(&file_stat->st_mtime));  // 显示修改时间
}
...
void recursive_ls(const char *path, int show_all, int show_details, int show_inode, int show_size, int depth) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (S_ISDIR(file_stat.st_mode)) {
            printf("\nDirectory: %s\n", new_path);
            recursive_ls(new_path, show_all, show_details, show_inode, show_size, depth + 1);  // 递归调用
        } else {
